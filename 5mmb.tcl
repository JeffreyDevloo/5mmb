...
# Using these variables freely throughout all procedures. Enclosure through the namespace
set struct_toon { account passwd name role raids}
interp alias {} struct_toon@  {} lsearch $struct_toon
		...
		while { [gets $tL line] >= 0 } {
			...
			switch $first_word {
				"box" {
					...
					lappend ::toons_l {$account $passwd $name $role $raids}
				}
				"keyboard" {
...
proc get_raid_hashes {} {
	set monitor $::monitor
	set use2monitors $::use2monitors
	array unset raidhash
	# 20 Window Raid
	switch monitor {
		"4k" {
		if { $use2monitors } {
